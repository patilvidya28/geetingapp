/**
 * Browser entry: inlined app code adapted from `src/app.js`.
 */
...
function initApp(doc) {
  ...
  function setGreeting() {
    ...
    greetingOutput.offsetHeight; // force reflow
  ...
  function triggerConfetti() {
    ...
    for (let i = 0; i < pieceCount; i += 1) {
      ...
      piece.style.setProperty("--spin", `${Math.random() > 0.5 ? "" : "-"}${180 + Math.random() * 180}deg`);
  ...
  function triggerPartyPopper() {
    ...
    container.addEventListener("animationend", () => {
      if (container.parentElement && container.childElementCount === 0) container.parentElement.removeChild(container);
    }, { once: true });
  ...
  function triggerGlowBurst() {
    ...
    const cleanup = (node) => { if (node.parentElement) node.parentElement.removeChild(node); };
  ...
  nameInput.addEventListener("keydown", (event) => { if (event.key === "Enter") handleGreetClick(event); });

  return { setGreeting, triggerRandomAnimation, _clearAnimations: clearAnimations };
}

if (typeof window !== "undefined") {
  window.addEventListener("DOMContentLoaded", () => { initApp(document); });
}
